...
async function startBot() {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') console.log('✅ Gamage-Gems AI IS ONLINE!');
        if (connection === 'close') {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (orderStates[sender]?.step === 'WAITING_FOR_ADDRESS') {
            ...
            // Match the exact format of your Gamage-Gems Admin Panel
            const GamageGemsOrder = {
                userId: "whatsapp_" + customerWaNumber,
                userEmail: "whatsapp@gamagegems.com",
        ...
        if (text.startsWith("order ")) {
            ...
            await sock.sendMessage(sender, { text: "🛒 *How to order:* \nPlease type 'order' followed by the gem name. \nExample: *order gems*" });
        }
        
        // --- DYNAMIC MENU FEATURE ---
        else if (text.includes("menu") || text.includes("price") || text.includes("list") || text.includes("gem")) {
            const currentMenu = await getMenuFromApp();
            ...
            let menuMessage = " *Gamage-Gems LIVE MENU* \n\n";
        ...
        // --- GREETINGS ---
        else if (text.includes("hi") || text.includes("hello") || text.includes("hey") || text.includes("Hi")) {
            await sock.sendMessage(sender, { text: "👋 *Welcome to Gamage-Gems!* \n\nI am your AI Assistant. Type *menu* to see our Gems Store, or type *order [Gem]* to buy instantly!" });
        }
        else if (text.includes("contact") || text.includes("call")) {
            await sock.sendMessage(sender, { text: "📞 *Contact Gamage-Gems:* \n\n- *Email:* sachilagamage@gmail.com" });
        }
        else {
            await sock.sendMessage(sender, { text: "🤔 I didn't quite catch that.\n\nType *menu* to see our Gems list, or *order [Gems]* to place an order!" });
        }
    });
}
